...
class HAT32SpectralEmbedding(nn.Module):
    """
    ONNX opset 12 friendly 频带 token 分支。

    目的：
        用固定 RFFT/DFT 基矩阵替代 torch.fft.rfft / torch.fft.rfftfreq。
        导出 ONNX 后主要变成 MatMul / Mul / Add / ReduceMean / Log 等基础算子。
    ...
    注意：
        该实现按固定 n_times 构造频率基矩阵，适合你的 [B, 32, 512] 固定长度部署。
    """
    ...
    def __init__(
        ...
        # imag = x @ imag_basis
        #
        # 对应 torch.fft.rfft(x * hann_window, dim=-1)
        # rfft[k] = sum_n x[n] * exp(-j * 2pi*k*n/N)
        # ============================================================
        t = self.n_times
        n = torch.arange(t, dtype=torch.float32)

        # 与 torch.hann_window(t) 默认 periodic=True 保持一致
        window = torch.hann_window(t, periodic=True, dtype=torch.float32)

        n_freq = t // 2 + 1
        freqs = torch.arange(n_freq, dtype=torch.float32) * (self.sfreq / float(t))

        real_cols = []
        imag_cols = []
        selected_band_ids = []

        for band_id, (low, high) in enumerate(self.bands):
            idx = torch.nonzero(
                (freqs >= float(low)) & (freqs < float(high)),
                as_tuple=False,
            ).flatten()

            for k in idx.tolist():
                angle = 2.0 * math.pi * float(k) * n / float(t)

                real_cols.append(window * torch.cos(angle))
                imag_cols.append(-window * torch.sin(angle))
                selected_band_ids.append(band_id)

        if len(real_cols) == 0:
            raise ValueError(
                "No FFT bins selected by frequency bands. Please check sfreq, n_times and bands."
            )

        real_basis = torch.stack(real_cols, dim=1)  # [T, F_selected]
        imag_basis = torch.stack(imag_cols, dim=1)  # [T, F_selected]

        band_counts = torch.zeros(self.num_bands, dtype=torch.float32)
        for band_id in selected_band_ids:
            band_counts[band_id] += 1.0

        band_weight = torch.zeros(
            len(selected_band_ids),
            self.num_bands,
            dtype=torch.float32,
        )

        for row, band_id in enumerate(selected_band_ids):
            band_weight[row, band_id] = 1.0 / band_counts[band_id]

        # persistent=False：不进入 state_dict，便于兼容旧 checkpoint
        self.register_buffer("real_basis", real_basis, persistent=False)
        self.register_buffer("imag_basis", imag_basis, persistent=False)
        self.register_buffer("band_weight", band_weight, persistent=False)

    def forward(self, x):
        # x: [B, C, T]
        b, c, t = x.shape

        # ONNX 导出时跳过 Python shape 分支，避免 TracerWarning。
        # 普通 PyTorch 推理时仍然做检查，防止误输入。
        if not torch.onnx.is_in_onnx_export():
            ...
            if t != self.n_times:
                raise ValueError(
                    f"HAT32SpectralEmbedding expects {self.n_times} time points, but got {t}."
                )

        x_float = x.float()

        real_basis = self.real_basis.to(dtype=x_float.dtype)
        imag_basis = self.imag_basis.to(dtype=x_float.dtype)
        band_weight = self.band_weight.to(dtype=x_float.dtype)

        # [B, C, T] @ [T, F_selected] -> [B, C, F_selected]
        real = torch.matmul(x_float, real_basis)
        imag = torch.matmul(x_float, imag_basis)

        power = real * real + imag * imag

        # [B, C, F_selected] @ [F_selected, num_bands] -> [B, C, num_bands]
        band_power = torch.matmul(power, band_weight)

        band_power = torch.log(torch.clamp(band_power, min=1e-8))

        # 用 mean + sqrt(var) 替代 torch.std，减少 ONNX 导出风险
        mean = band_power.mean(dim=(1, 2), keepdim=True)
        centered = band_power - mean
        var = (centered * centered).mean(dim=(1, 2), keepdim=True)
        std = torch.sqrt(torch.clamp(var, min=1e-10))

        band_power = centered / std
