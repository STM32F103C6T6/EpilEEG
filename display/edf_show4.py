...
class EDFViewer:
    def __init__(self, eeg_root):
        ...
        self.raw_orig = None       # 原始读入
        self.raw_proc = None       # 预处理后
        self.channels = []
    ...
    def _build_ui(self):
        ...
        self.fig.canvas.manager.set_window_title("EDF EEG Browser with Filtering + Metrics")

        self.ax_file_list = self.fig.add_axes([0.02, 0.56, 0.18, 0.34])
        self.ax_file_slider = self.fig.add_axes([0.205, 0.56, 0.012, 0.34])

        self.ax_ch_list = self.fig.add_axes([0.02, 0.18, 0.18, 0.26])
        self.ax_ch_slider = self.fig.add_axes([0.205, 0.18, 0.012, 0.26])

        self.ax_checks = self.fig.add_axes([0.23, 0.17, 0.10, 0.18])
        self.ax_plot = self.fig.add_axes([0.35, 0.56, 0.43, 0.32])
        self.ax_psd = self.fig.add_axes([0.35, 0.17, 0.43, 0.25])
        self.ax_metrics = self.fig.add_axes([0.80, 0.17, 0.18, 0.72])

        self.ax_metrics.set_title("Metrics / Hints", fontsize=11)
        self.ax_metrics.axis("off")

        self.ax_time_slider = self.fig.add_axes([0.40, 0.08, 0.30, 0.03])

        ax_zoom_in = self.fig.add_axes([0.73, 0.06, 0.04, 0.05])
        ax_zoom_out = self.fig.add_axes([0.78, 0.06, 0.04, 0.05])
        ax_reload = self.fig.add_axes([0.23, 0.10, 0.10, 0.05])

        self.btn_zoom_in = Button(ax_zoom_in, "+")
        self.btn_zoom_out = Button(ax_zoom_out, "-")
        self.btn_reload = Button(ax_reload, "Reprocess")

        self.btn_zoom_in.on_clicked(self.on_zoom_in)
        self.btn_zoom_out.on_clicked(self.on_zoom_out)
        self.btn_reload.on_clicked(self.on_reprocess)
    ...
    def _detect_bad_channels(self, raw, flat_thresh=1e-12, std_z_thresh=5.0, max_bad_ratio=0.3):
        ...
        try:
            ...
            if len(bads) > max_bad_num:
                print(f"Warning: too many bad channels detected ({len(bads)}), ignore auto bads.")
                return []
    ...
    def _preprocess_raw(self, raw):
        ...
        if self.enable_interp and len(raw.info["bads"]) > 0:
            try:
    ...
    def on_reprocess(self, event):
        self.raw_proc, self.auto_bads = self._preprocess_raw(self.raw_orig)
        self.channels = self.raw_proc.ch_names
        self.channel_index = min(self.channel_index, len(self.channels) - 1)
        self.ch_list.set_items(self.channels, selected_index=self.channel_index)
        self._reset_ch_slider()
        self._reset_time_slider()
        self.plot_signal()
    ...
    def _get_current_window(self):
        ...
        data_proc, times = self.raw_proc[self.channel_index, start_sample:stop_sample]
        signal_proc_uv = data_proc[0] * 1e6

        if self.channel_index < len(self.raw_orig.ch_names) and self.raw_proc.ch_names[self.channel_index] in self.raw_orig.ch_names:
            ch_name = self.raw_proc.ch_names[self.channel_index]
            orig_idx = self.raw_orig.ch_names.index(ch_name)
            data_orig, _ = self.raw_orig[orig_idx, start_sample:stop_sample]
            signal_orig_uv = data_orig[0] * 1e6
        else:
            signal_orig_uv = None

        return signal_orig_uv, signal_proc_uv, times, sfreq
    ...
    def plot_signal(self):
        ...
        signal_orig_uv, signal_proc_uv, times, sfreq = self._get_current_window()
        metrics = self._compute_metrics(signal_proc_uv, sfreq)

        if signal_orig_uv is not None:
            self.ax_plot.plot(times, signal_orig_uv, linewidth=0.7, alpha=0.45, label="Original")

        self.ax_plot.plot(times, signal_proc_uv, linewidth=0.9, label="Processed")

        if metrics is not None and len(metrics["spike_idx"]) > 0:
            spike_times = times[metrics["spike_idx"]]
            spike_vals = signal_proc_uv[metrics["spike_idx"]]
        ...
        total_duration = self.raw_proc.times[-1] if len(self.raw_proc.times) > 0 else 0
        ...
        if metrics is not None and metrics["band_info"] is not None:
            ...
            self.ax_psd.set_title("Power Spectrum (processed)")
