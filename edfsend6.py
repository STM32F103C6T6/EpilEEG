...
# --- 用户配置区 ---
# 包含 (N, 32, 512) 形状数据的NPY文件
NPY_DATA_PATH = 'processed_data/epilepsy_eeg/epilepsy_filter_epoch/Patient_004_epochs.npy'
ONNX_MODEL_PATH = 'experiment_outputs/onnx_models_cpu/epilepsy_eeg_epilepsy_filter_epoch_HAT_seed666_cpu.onnx'  # 你的ONNX模型文件名
SERIAL_PORT = 'COM5'
BAUDRATE = 115200*8
...
def main():
    ...
    if not os.path.exists(NPY_DATA_PATH):
        ...
        # 为了方便测试，我们可以创建一个示例文件
        response = input("是否要创建一个示例NPY文件 (10个32x512的矩阵)? (y/n): ").lower()
        if response == 'y':
            print("正在创建示例文件...")
            sample_data = np.random.randn(10, 32, 512).astype(np.float32)
    ...
    try:
        ...
        # 验证数据形状
        if all_eeg_chunks.ndim != 3 or all_eeg_chunks.shape[1] != 32 or all_eeg_chunks.shape[2] != 512:
            print(f"错误：NPY文件中的数据形状为 {all_eeg_chunks.shape}，不符合预期的 (N, 32, 512) 格式。")
            return
        num_chunks = all_eeg_chunks.shape[0]
        print(f"数据加载成功，共包含 {num_chunks} 个 32x512 的矩阵。")
    except Exception as e:
        print(f"加载NPY文件失败: {e}")
        return
    ...
    try:
        ...
        for i, eeg_chunk_np in enumerate(all_eeg_chunks):
            ...
            dt = t_end - t_start
            bps_eff = (packet_len * 10) / dt

            print(f"[Send Timing] "
                  f"bytes={packet_len}, "
                  f"time={dt:.6f}s, "
                  f"eff_baud={bps_eff:.0f}bps, "
                  f"target={BAUDRATE}bps")
