...
def main():
    ...
    # 1. 我们不再使用 np.arange(float)，因为它有精度问题。
    #    我们直接创建一个字节数组。
    payload_size = 33 * 512 * 4  # 67584 字节

    # 2. 生成一个简单的、可预测的字节序列：0, 1, 2, ..., 255, 0, 1, ...
    #    (i % 256) 确保字节值在 0-255 之间循环
    eeg_payload_list = [(i % 256) for i in range(payload_size)]

    eeg_payload = bytes(eeg_payload_list)
